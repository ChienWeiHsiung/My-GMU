...
class System:
    def __init__(self, P=100000, change_time=100000):
        ...
        self.q1 = Queue(0.30, 5, P)
        self.q2 = Queue(0.25, 5, P)
        self.qb = Queue(0.40, 0, P)
        ...
        self.T = 1000
        self.k = 3
        self.prob = [[0 for x in range(3)] for y in range(4)]
        ...
        #state : (0,0)=0 (0,1)=1, (1,0)=2, (1,1)=3 | (q1, q2) => binary q1q2
        #action: 0 = take packet from queue1, 1 = queue2, 2 = best effort queue
        self.Qtable = [[0 for x in range(3)] for y in range(4)]
        
        #new parameter
        self.requirements = [self.q1.delay_requirement, self.q2.delay_requirement]
    ...
    def Qlearning_greedy(self, iteration):
        ...
        for i in range(iteration):
            ...
            if i == self.change_time:
                self.q1.change(0.20, 2)
                self.q2.change(0.20, 3)
                self.qb.change(0.55, 0)
                self.requirements = [self.q1.delay_requirement, self.q2.delay_requirement]
            ...
            #Compute reward
            reward = self.reward(action, state, next_state, i)
            #Update Q table
    ...
    def all_queue_check(self):
        if self.q1.isEmpty() and self.q2.isEmpty() and self.qb.isEmpty():
            return True
        else:
            return False
    ...
    def check_action_greedy(self, action, state):
        ...
        temp = [self.q1.isEmpty(), self.q2.isEmpty(), self.qb.isEmpty()]
        if temp[action] == True:
            alter_action = 0
    ...
    def action_batch(self, state):
        ...
        if (state % self.T) == 0:
            for i in range(4):
                k = self.k
                k_pow = [pow(k, self.Qtable[i][0]), pow(k, self.Qtable[i][1]), pow(k, self.Qtable[i][2])]
                self.prob[i] = [k_pow[0]/sum(k_pow), (k_pow[0]+k_pow[1])/sum(k_pow), 1]
    ...
    def check_action_batch(self, action, state):
        ...
        if temp[action] == True:
            ...
            while temp[alter_action] == True:
                alter_action = random.randint(0, 2)
            return alter_action
        else:
            return action
    ...
    def reward(self, action, state, next_state, time):
        #Compute reward
        #rtime,1
        r1 = 0
        if self.q1.mean_delay < self.q1.delay_requirement:
        ...
        if self.state_compute(next_state) < self.state_compute(state):
            rstate = self.C3
        reward = rtime + rstate
        return reward
    
    def state_compute(self, state):
        sum = 0
        #modified
        temp = "{0:02b}".format(state)
        #print(temp)
        #print(self.requirements)
        for i in range(len(temp)) :
            #print(temp[i])
            if temp[i] == '1':
                sum += 1/self.requirements[i]
        #print(sum)
        return sum
