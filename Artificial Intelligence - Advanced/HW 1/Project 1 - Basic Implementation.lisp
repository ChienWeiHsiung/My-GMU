...
to provide the current alpha value."
  (setf (aref q-table current-state action) ;;Q(s,a)
	(+ (* (- 1 (funcall alpha-func iteration)) (aref q-table current-state action)) ;;(1-alpha)*Q(s,a)
	   (* (funcall alpha-func iteration) (+ reward (* gamma (max-q q-table next-state)) ) ) ;;alpha*(r+gamma*maxQ(s',a'))
	 )
	)
  q-table
)
...
(defun learn-nim (heap-size gamma alpha-func num-iterations)
  ...
  (let ( (q-table (make-q-table (+ heap-size 6) 3));;make a table
	 (state)(old-state)(reward)(my-move) ) 
    (dotimes (i num-iterations)          ;;Iterations times
      (setf state 0) ;; Set state to 0
      (loop
	(setf old-state state) ;;old state <- state
	(setf my-move (max-action q-table state)) ;;my-move (my-action)
	(setf state (+ state (+ 1 my-move))) ;;update state ( sticks removed are equal to action + 1)
	(setf reward 0) ;;set reward to 0 (If no reward has been set yet, set it to 0)
	(when (= state heap-size) ;;If I lost, set my reward to -1
	  (setf reward -1))
	(setf state (+ state (+ 1 (max-action q-table state)))) ;;Determine and make my opponent's move, update state
	(when (= state heap-size) ;;If the opponent lost, set my reward to +1
	  (setf reward 1))
	;;If new state is bigger than the heap size, exit loop.
	   ...
	   (let ( (answer (ask-if-user-goes-first)) (state 0) computer-action)
		 ...
		 (progn
		   (loop
		     ...
		     (setf state (+ state (make-user-move)) ) ;;user move
		     (when (> state (- heap-size 1))
		       (format t "~%You Lose")
		       (return))
		     (setf computer-action (+ 1 (max-action q-table state)))
		     (setf state (+ state computer-action) ) ;;computer move
...
;;It'll print best actions of State 0 to State N-1, State N is ignored.
...
(defparameter *my-q-table* (learn-nim 22 0.1 #'basic-alpha 50000))
;; OUTPUT
;; *MY-Q-TABLE*
