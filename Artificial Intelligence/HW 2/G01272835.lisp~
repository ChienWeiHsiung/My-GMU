...
(defun net-error (output correct-output)
  ...
  (car (car (scalar-multiply 0.5 (multiply (transpose (subtract correct-output output)) (subtract correct-output output) ))))
  )
...
the datum as input."
  (let ((h (mapcar (lambda (x)  (list (sigmoid (car x))))  (multiply v (car datum))))) ;;compute h first
    (let ((o (mapcar (lambda (x)  (list (sigmoid (car x))))  (multiply w h))))  ;;then compute o
      o)) ;;return o
  )
...
returning a list consisting of new, modified V and W matrices."
  (let ((o (forward-propagate datum v w)) (h (mapcar (lambda (x)  (list (sigmoid (car x))))  (multiply v (car datum)))) ) ;;get o and h
    (let ((odelta (e-multiply (e-multiply (subtract (car (cdr datum)) o) o) (subtract-from-scalar 1 o))))    ;; compute odelta
      (let ((hdelta (e-multiply (e-multiply h (subtract-from-scalar 1 h)) (multiply (transpose w) odelta) )))  ;;compute hdelta
	(setf w (add w (scalar-multiply alpha (multiply odelta (transpose h)))))                     ;;update w
	(setf v (add v (scalar-multiply alpha (multiply hdelta (transpose (car datum))))))           ;;update v
	(list v w)
	))) ;;return o
  )
...
(defun net-build (data num-hidden-units alpha initial-bounds max-iterations modulo &optional print-all-errors)
  (let ((v (make-random-matrix num-hidden-units (length (car (car data))) initial-bounds))           ;;create v
	(w (make-random-matrix (length (car (cdr  (car data)))) num-hidden-units initial-bounds)))   ;;create w
    (dotimes (i max-iterations)
      (dolist (datum (shuffle data))  ;;shuffle the data
	(let ((temp (back-propagate datum alpha v w))) ;;run backpropagation
	  (setf v (car temp))          ;;get the new v
	  (setf w (car (cdr temp)))))  ;;get the new w
      (when (eq 0 (mod (+ 1 i) modulo))  ;;every modulo iterations
	(let ((err (mapcar (lambda (datum) (net-error (forward-propagate datum v w) (car (cdr datum))))  data))) ;;get error
	  (when (eq print-all-errors t)
	    (format t "Errors : ~a~%" err))
	  (format t "Worst error : ~f~%" (reduce #'max err))
	  (format t "Mean error : ~f~%" (average err))
	  (when (<= (reduce #'max err) *a-good-minimum-error* ) ;;error condition
	    (return)))))
    (list v w))
  )
...
(defun simple-generalization (data num-hidden-units alpha initial-bounds max-iterations)
  ...
  (let ((train (subseq data 0 (floor (/ (length data) 2)))) (test (subseq data (floor (/ (length data) 2))))) ;;first half as train, second half as test
    (let* ((temp (net-build train num-hidden-units alpha initial-bounds max-iterations max-iterations)) ;;run backpropagation with first half and get the v, w
      ...
      (let ((err (mapcar (lambda (datum) (net-error (forward-propagate datum v w) (car (cdr datum))))  test))) ;;run forwardpropagation with second half and get the error
	(average err))))
  )
...
A : It'll run 20000 times because it can't hit the error condition.
...
A : Average mean error is around 0.00014323472.
    I try num-hidden-units = 15, get a better result around 0.00009366969. 
          num-hidden-units = 20, also the same as num-hidden-units = 15
    with max-iterations = 10000, result is around 0.00005166974
...
A : Average error among samples in the second half
   (simple-generalization *voting-records* 10 1.0 2 10000) : around 0.02614611
   (simple-generalization *voting-records* 15 1.0 2 5000)  : around 0.023615712
   (simple-generalization *voting-records* 20 1.0 2 10000) : almost the same as num-hidden-units=15.
   (simple-generalization *voting-records* 25 1.0 2 10000) : around 0.021322766 
My best : (simple-generalization *voting-records* 15 1.0 2 10000) : around 0.019296229

   
;;; how well does this generalize usually?  Can you modify it to typically generalize better?
(simple-generalization *mpg* ...) ;; pick appropriate values
A : (simple-generalization *mpg* 10 1.0 2 5000) : around 0.0054553207
    (simple-generalization *mpg* 20 1.0 2 5000)  : around 0.00447252
    (simple-generalization *mpg* 15 1.0 2 10000) : around 0.0061160616
    (simple-generalization *mpg* 15 1.0 2 15000) : around 0.0049085184
My best : (simple-generalization *mpg* 15 1.0 2 5000)  : around 0.0043019447

;;; how well does this generalize usually?  Can you modify it to typically generalize better?
(simple-generalization *wine* ...)  ;; pick appropriate values
A : (simple-generalization *wine* 10 1.0 2 10000): around 0.32949102
    (simple-generalization *wine* 15 1.0 2 5000) : around 0.31659335
    (simple-generalization *wine* 20 1.0 2 5000) : around 0.33061397
My best : (simple-generalization *wine* 10 1.0 2 5000) : around 0.3005867 


;;;;Report :
   Although the professor mentioned that we should finish it immediately, I still spent some time on it.
   The biggest problem I encountered is to comprehend the data types, structures and function :
      First, the formulas for backpropagation and forward propagation.
      Second, operations like ¡§.¡¨, no dot.
      Finally, the way to present matrix for w, v, input vector, and output vector.
   I tried all the functions and matrix type one by one to make sure that I was on the right way.
   After I understood these, it became easier to code indeed.
   By this homework, I learn more about this concept. Before this, it was just a ambiguous concept to me. I understand what professor taught on course but I can¡¦t connect it to programming.
   Therefore, I'm glad that I completed this.

   By the way, the k-fold-validation implementation, approach, additional data, and some results using k-fold-validation is in my "additional.lisp" file.
   Please check it out.
